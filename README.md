...
- `regime` n'accepte qu'une liste fermée de valeurs (`vegetarien`, `vegan`, `pescatarien`, `sans-gluten`, `sans-lactose`, `sans-porc`, `sans-fruits-a-coque`). Une valeur inconnue fait échouer le build en listant les valeurs acceptées. La liste s'étend dans `src/content.config.ts`. `vegetarien` autorise œufs et laitages ; `vegan` non ; `pescatarien` autorise le poisson mais pas la viande. L'accueil dérive un bouton « Sans viande » qui réunit `vegan`, `vegetarien` et `pescatarien` : ce n'est pas une valeur à saisir.
