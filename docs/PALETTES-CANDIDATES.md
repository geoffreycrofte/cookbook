...
Objectif : remplacer l'ancienne palette (« Papier & braise », crème + brun +
terracotta) jugée trop générique.

**Palette active : Tomate & basilic**, recopiée dans `src/styles/global.css`,
`scripts/verifier-contrastes.mjs` et `src/lib/theme.ts`.

Les trois jeux de tokens sont conservés dans `src/styles/themes/` comme
référence pour un futur sélecteur de thème côté visiteur (non implémenté) :

| Palette | Fichier | Identité | Source | État |
| --- | --- | --- | --- | --- |
| Tomate & basilic | `src/styles/themes/tomate-basilic.css` | Rouge tomate franc + vert basilic | Composée à la main, ratios calculés | **active** |
| Rumi | `src/styles/themes/rumi.css` | Violet profond + ambre | Figma + matrice | de côté |
| Soda Pop | `src/styles/themes/soda-pop.css` | Jaune fluo + magenta fraise | Figma + matrice | de côté |
